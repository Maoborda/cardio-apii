"""
Cardio-IA Latam — API FastAPI (versión ligera sin TensorFlow)
El modelo se carga solo si está disponible en el servidor.
"""
...
# Intentar cargar TensorFlow solo si está disponible
try:
    import tensorflow as tf
    ruta = os.getenv("RUTA_MODELO", "cardio_brain_mejor.keras")
    if os.path.exists(ruta):
        MODELO = tf.keras.models.load_model(ruta)
        print(f"✅ Modelo TensorFlow cargado: {ruta}")
    else:
        print(f"⚠️ Archivo del modelo no encontrado: {ruta}")
except ImportError:
    print("⚠️ TensorFlow no instalado — API corre en modo demo")
...
try:
    ...
    if os.path.exists(ruta_meta):
        ...
        METADATOS = {"frecuencia_muestreo": 360.0, "longitud_ventana": 540, "umbral_confianza": 0.70}
except Exception:
    METADATOS = {"frecuencia_muestreo": 360.0, "longitud_ventana": 540, "umbral_confianza": 0.70}
...
def demo_diagnostico():
    """Respuesta de demostración cuando el modelo no está cargado"""
    probs = np.array([0.82, 0.05, 0.04, 0.03, 0.03, 0.03])
    return {**diagnosticar(probs), "modo": "DEMO — modelo no cargado en este servidor"}
